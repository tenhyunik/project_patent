...
class APIRateMonitor:
    def __init__(self, metrics: DesignMetrics, window_size: int = 5):
...
class TokenBucketRateLimiter:
    def __init__(self, metrics: DesignMetrics, rate_limit: int = 45, 
...
class XMLBuilder:
    ...
    def get_summary(self):
        return f"{self.filename}: 총 {self.design_count:,}개 디자인"
        ...
        try:
            async with self._lock:
                ...
                for source_item in items:
                    ...
                    fields = [
                        'number', 'articleName', 'applicantName',
                        'inventorName', 'agentName', 'applicationNumber',
                        'applicationDate', 'openNumber', 'openDate',
                        'registrationNumber', 'registrationDate', 'publicationNumber',
                        'publicationDate', 'applicationStatus', 'priorityNumber',
                        'priorityDate', 'designMainClassification', 'dsShpClssCd',
                        'fullText'
                    ]
                    ...
                    if self.design_count - self.last_save_count >= self.save_threshold:
                        self.save()
                        self.last_save_count = self.design_count
                        self.logger.info(f"중간 저장 완료: {self.design_count:,}개 디자인")
    ...
    def save(self):
        try:
            ...
            self.logger.info(f"XML 저장 완료: {self.filename} (총 {self.design_count:,}개)")
        except Exception as e:
...
class ProgressTracker:
    def __init__(self, table_type: str):
        os.makedirs('data', exist_ok=True)
        self.progress_file = f'data/progress_design_{table_type}.json'
...
class AsyncDesignDownloader:
    def __init__(self, table_types: List[str]):
        ...
        for table_type in table_types:
            if table_type == 'TB24_200':
                self.builders[table_type] = XMLBuilder(f'{date_str}_design_corp.xml')
                self.table_names[table_type] = 'tb24_200_corp_applicant'
            else:
                self.builders[table_type] = XMLBuilder(f'{date_str}_design_univ.xml')
    ...
    def setup_logging(self):
        ...
        log_file = f'logs/design_download_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ...
        try:
            ...
            async with self.session.get(self.base_url, params=params) as response:
                ...
                if "Repetition Request ERROR" in response_text:
                    self.logger.error(f"API error for {applicant_no}: Rate limit exceeded")
            ...
            self.logger.error(f"Exception in fetch_design_data for {applicant_no}: {str(e)}")
            self.metrics.api_errors.inc()
            return None
        finally:
            # 요청 시간이 긴 경우 로그로 기록
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        success = await self.builders[table_type].add_design_data(xml_content, applicant_no)
                        if success:
            ...
            try:
                ...
                if completed % 5 == 0:
                    progress_message = (
                        ...
                        f"(전체 디자인 수: {self.builders[table_type].design_count:,}개)"
                    )
                    self.logger.info(progress_message)
            except Exception as e:
                self.logger.error(f"[{table_type}] 배치 {batch_index}의 작업 실패: {str(e)}")
        
        if self.builders[table_type].design_count > 0:
    ...
    - 처리된 디자인 수: {self.builders[table_type].design_count:,}개
    ...
    """)
            ...
            self.logger.info(f"""
    === [{table_type}] 처리 완료 ===
    - 최종 처리된 디자인 수: {self.builders[table_type].design_count:,}개
    ...
    async def process_all(self):
        try:
            ...
            for table_type in self.builders.keys():
                task = asyncio.create_task(self.process_type(table_type))  # process_table -> process_type
                tasks.append(task)
            ...
            for table_type in self.builders.keys():
                if self.failed_requests[table_type]:
                    await self.retry_failed_requests(table_type)
...
async def main():
    try:
        downloader = AsyncDesignDownloader(['TB24_200', 'TB24_210'])
