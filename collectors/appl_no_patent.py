...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(  # File handler
            filename=f"/home/ubuntu/airflow/logs/patent_processing_{datetime.now().strftime('%Y%m%d')}.log"
        )
    ]
)
logger = logging.getLogger('patent_processing')
...
def main():
    try:
        ...
        BASE_URL = "http://plus.kipris.or.kr/kipo-api/kipi/patUtiModInfoSearchSevice/getChangeInfoSearch"
        ...
        try:
            ...
            # CSV 파일 경로 설정
            corp_csv_path = output_dir / f"patent_corp_appl_no_{today_date}.csv"
            univ_csv_path = output_dir / f"patent_univ_appl_no_{today_date}.csv"
