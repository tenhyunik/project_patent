...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(  # File handler
            filename=f"/home/ubuntu/airflow/logs/trademark_processing_{datetime.now().strftime('%Y%m%d')}.log"
        )
    ]
)
logger = logging.getLogger('trademark_processing')
...
def main():
    try:
        ...
        BASE_URL = "http://plus.kipris.or.kr/kipo-api/kipi/trademarkInfoSearchService/getChangeInfoSearch"
        ...
        try:
            ...
            # CSV 파일 경로 설정
            corp_csv_path = output_dir / f"trademark_corp_appl_no_{today_date}.csv"
            univ_csv_path = output_dir / f"trademark_univ_appl_no_{today_date}.csv"
