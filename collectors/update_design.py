...
class DesignAPIProcessor:
    def __init__(self, access_key: str, rate_limit: int = 50, max_retries: int = 3):
        """
        디자인 API 처리기 초기화
        ...
        self.base_url = "http://plus.kipris.or.kr/openapi/rest/designInfoSearchService/applicationNumberSearchInfo"
        self.session = requests.Session()
        self.logger = logging.getLogger(__name__)
        self.logger.info(f"DesignAPIProcessor 초기화 완료 (rate_limit: {rate_limit})")
    ...
    def fetch_design_data(self, application_number: str) -> Dict:
        """
        디자인 데이터 조회
        ...
        try:
            ...
            design_info = root.find('.//DesignInfo')
            ...
            field_mapping = {
                ...
                'openDate': 'openDate',
                'designMainClassification': 'designMainClassification',
            ...
            data = {
                key: design_info.findtext(xml_tag, '')
...
class DesignDataProcessor:
    def __init__(self, access_key: str, base_dir: Path, logger: logging.Logger):
        """
        디자인 데이터 처리기 초기화
        ...
        """
        self.api_processor = DesignAPIProcessor(access_key)
    ...
    def process_chunk(self, chunk: pd.DataFrame, output_path: Path) -> List[Dict]:
        ...
        for appl_no in chunk['appl_no']:
            data = self.api_processor.fetch_design_data(str(appl_no))
            if data:
                results.append(data)
        return results
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger('design_update_processing')
    ...
    file_handler = logging.FileHandler(
        log_dir / f"design_update_{datetime.now().strftime('%Y%m%d')}.log"
    )
...
def main():
    ...
    try:
        ...
        # 입출력 경로 설정
        input_paths = {
            "corp": base_dir / f"design_corp_appl_no_{today}.csv",
            "univ": base_dir / f"design_univ_appl_no_{today}.csv"
        }

        output_paths = {
            "corp": base_dir / f"design_corp_update_{today}.csv",
            "univ": base_dir / f"design_univ_update_{today}.csv"
        }
        
        # 데이터 처리기 생성
        processor = DesignDataProcessor(access_key, base_dir, logger)
